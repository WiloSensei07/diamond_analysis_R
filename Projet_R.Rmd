...
Pour la prédiction, nous allons utiliser le modèle final qui a été fourni grâce à la sélection effectuée avec le critère BIC.

```{r}
pred <- predict(final_model, data = test_data) # Prédictions normalisées
```
Et nous avons obtenu les prédictions suivantes pour les 06 pemiers tests :
```{r}
