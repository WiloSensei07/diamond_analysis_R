...
### 1- La variable dépendante: Carat
...
### Test d'indépendance entre la variable à expliquer (Price) et les variables quantitatives
...
donnees_num <- donnees[sapply(donnees, is.numeric)]
donnees_num <- subset(donnees_num, select = -...1)
coef_donnees_num <- cov(donnees_num)^2
print(coef_donnees_num)
```
...
## MODÉLE
```{r}
model <- lm(price ~ carat + depth + table + x + y + z, data = donnees)
summary(model)
vif_model <- vif(model)
print(vif_model)
```

### Petite conclusion sur le modèle
#### Coefficients
```{r}
print("Carat : Avec un coefficient de 10686.309 et une valeur p très faible (< 2e-16), il est clair que le carat est un prédicteur extrêmement puissant du prix. Plus le diamant est lourd, plus son prix est élevé.
...
```{r}
print("R-squared (R²) : Le R² de 0.8592 indique que le modèle explique environ 85.92% de la variabilité du prix des diamants, ce qui est très élevé. Cela suggère que le modèle est globalement efficace pour prédire le prix.
...
#### Conclusion
```{r}
print("Le modèle est statistiquement robuste et efficace pour prédire le prix des diamants en se basant sur les variables incluses. Cependant, l'interprétation de certains coefficients (notamment pour les dimensions x, y, et z) nécessite une attention particulière, car ils peuvent être affectés par la multicollinéarité entre ces variables. Il serait judicieux de vérifier la présence de multicollinéarité, par exemple en calculant le VIF (Variance Inflation Factor) pour ces variables.")
```

#### Axes d'amélioration
```{r}
print("Pour améliorer ce modèle, on pourrait envisager d'exclure ou de transformer certaines variables, d'ajouter des interactions si pertinentes, ou d'explorer des modèles non linéaires ou des méthodes d'ensemble si la complexité des relations le justifie.")
```


## V- Traitement des valeurs manquantes & valeurs abbérantes
